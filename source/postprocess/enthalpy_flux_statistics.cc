...
#include <aspect/postprocess/enthalpy_flux_statistics.h>
...
namespace aspect
{
  namespace Postprocess
  {
    template <int dim>
    void
    EnthalpyFluxStatistics<dim>::initialize ()
    {
    ...
    EnthalpyFluxStatistics<dim>::execute (TableHandler &statistics)
    {
            ...
            if (cell->at_boundary(f))
              {
                ...
                for (unsigned int q=0; q<fe_face_values.n_quadrature_points; ++q)
                  {
                    const double enthalpy = material_model.enthalpy(in.temperature[q],in.pressure[q],in.composition[q],in.position[q]);

                    local_normal_flux
                    +=
                      enthalpy * out.densities[q]
      ...
      for (std::map<types::boundary_id, double>::const_iterator
        ...
        {
          const std::string name = "Outward enthalpy flux through boundary with indicator "
      ...
      return std::pair<std::string, std::string> ("Enthalpy fluxes through boundary parts:",
                                                  screen_text.str());
    }
  }
}
...
namespace aspect
{
  namespace Postprocess
  {
    ASPECT_REGISTER_POSTPROCESSOR(EnthalpyFluxStatistics,
                                  "enthalpy flux statistics",
