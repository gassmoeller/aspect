...
#include <aspect/postprocess/visualization/rho_div_u.h>
...
namespace aspect
{
  namespace Postprocess
  {
    namespace VisualizationPostprocessors
    {
      ...
      RhoDivU<dim>::execute() const
      {
        std::pair<std::string, Vector<float> *>
        return_value ("rho_div_u",
          ...
          if (cell->is_locally_owned())
            {
              ...
              const double cell_mass_error = rho_div_u;
...
namespace aspect
{
  namespace Postprocess
  {
    namespace VisualizationPostprocessors
    {
      ASPECT_REGISTER_VISUALIZATION_POSTPROCESSOR(RhoDivU,
                                                  "rho div u",
                                                  "")
    }
  }
