...
#include <aspect/postprocess/visualization/u_grad_rho.h>
...
namespace aspect
{
  namespace Postprocess
  {
    namespace VisualizationPostprocessors
    {
      ...
      UGradRho<dim>::execute() const
      {
        std::pair<std::string, Vector<float> *>
        return_value ("u_grad_rho",
          ...
          if (cell->is_locally_owned())
            {
              ...
              const double cell_mass_error = u_grad_rho;
...
namespace aspect
{
  namespace Postprocess
  {
    namespace VisualizationPostprocessors
    {
      ASPECT_REGISTER_VISUALIZATION_POSTPROCESSOR(UGradRho,
                                                  "u grad rho",
                                                  "")
    }
  }
