...
namespace aspect
{
  namespace GeometryModel
  {
    ...
    /*template <int dim>
    void
    SphericalShell<dim>::initialize(const InitialTopographyModel::Interface<dim> *topo_pointer)
    {
      topo = topo_pointer;
    }*/
          ...
          if (custom_mesh == none)
          ...
          else
            {
              ...
              if (custom_mesh == list)
                {
                  R_values[0] = R0;
                  R_values[n_R_values-1] = R1+topo;
                  for (unsigned int s=1; s<(n_R_values-1); ++s)
                    R_values[s] = R_values_list[s-1] + s*topo/(n_R_values-1);
                }
