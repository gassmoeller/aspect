...
namespace aspect
{
  namespace Particle
  {
    ...
    template <int dim>
    std::map<double, typename parallel::distributed::Triangulation<dim>::active_cell_iterator>
    World<dim>::neighbor_cells_to_search(const Particle<dim> &particle,
                                         const typename parallel::distributed::Triangulation<dim>::active_cell_iterator &cell) const
    {
      std::map<double,typename parallel::distributed::Triangulation<dim>::active_cell_iterator> neighbor_cells;
        ...
        if (cell->at_boundary(face_no) == false)
          {
            if (cell->neighbor(face_no)->active())
              ...
              // If the neighbor cell is not active (i.e it has
              // children) cycle through them. If the children have
              // children we can ignore those, since they can not
              // be connected to our original cell, because level
              // differences across faces are limited to 1.
              for (unsigned int child_index=0; child_index<cell->neighbor(face_no)->n_children(); ++child_index)
                if (cell->neighbor(face_no)->child(child_index)->active())
                  {
                    const double center_distance = (particle.get_location() - cell->face(face_no)->center()).norm();
                    neighbor_cells.insert(std::make_pair(center_distance,cell->neighbor(face_no)->child(child_index)));
                  }
          }
          ...
          if (it->first != std::make_pair(-1,-1))
            {
              ...
              const std::map<double, typename parallel::distributed::Triangulation<dim>::active_cell_iterator>
