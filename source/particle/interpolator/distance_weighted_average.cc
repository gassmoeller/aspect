    ...
    namespace Interpolator
    {
      namespace internal
      {
        double weight (const double distance, const double cell_width)
        {
          ...
          return std::max(1.0 - std::pow((distance/cell_width),2.0),0.0);
          else
                                                            ...
                                                            const typename parallel::distributed::Triangulation<dim>::active_cell_iterator &cell) const
      {
        ...
        const double interpolation_support = cell_diameter / (std::pow(static_cast<double>(n_particles),1.0/dim));
        ...
        for (typename std::vector<Point<dim> >::const_iterator itr = positions.begin(); itr != positions.end(); ++itr, ++index_positions)
          {
            ...
            cell_properties[index_positions][property_index] /= integrated_weight;
          }
