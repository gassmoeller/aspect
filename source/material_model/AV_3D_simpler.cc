...
//Next session is a more evolved implementation of anisotropic viscosity in the material model based on Hansen et al 2016 and Kiraly et al 2020
  namespace MaterialModel
  {
    template <int dim>
    void
    AV_3D<dim>::set_assemblers(const SimulatorAccess<dim> &,
    ...
    initialize()
    {
      this->get_signals().set_assemblers.connect (std::bind(&AV_3D<dim>::set_assemblers,
                                                            std::cref(*this),
                                                            std::placeholders::_1,
                                                            std::placeholders::_2));
      AssertThrow((dim==3),
    ...
    template <int dim> 
    void
    AV_3D<dim>::evaluate (const MaterialModel::MaterialModelInputs<dim> &in,
                       MaterialModel::MaterialModelOutputs<dim> &out) const
    {
      ...
      AssertThrow((this->introspection().compositional_name_exists("euler1")),
                  ExcMessage("AV_3D material model only works if there is a compositional field called euler1 (first euler angle)."));
      AssertThrow(this->introspection().compositional_name_exists("euler2"),
                  ExcMessage("AV_3D material model only works if there is a compositional field called euler2 (second euler angle)."));
      AssertThrow(this->introspection().compositional_name_exists("euler3"),
                  ExcMessage("AV_3D material model only works if there is a compositional field called euler3 (third euler angle)."));
    ...
    template <int dim>
    bool
    AV_3D<dim>::is_compressible () const
    {
      return false;
    }
    ...
    template <int dim>
    double
    AV_3D<dim>::reference_viscosity () const
    {
      return 1e20;
    }
    ...
    template <int dim>
    void
    AV_3D<dim>::parse_parameters (ParameterHandler &prm)
    {
    ...
    template <int dim>
    void
    AV_3D<dim>::declare_parameters (ParameterHandler &prm)
    {
      prm.enter_subsection("Material model");
      {
        prm.enter_subsection("AnisotropicViscosity");
    ...
    template <int dim>
    void
    AV_3D<dim>::create_additional_named_outputs(MaterialModel::MaterialModelOutputs<dim> &out) const
    {
...
#define INSTANTIATE(dim) \
  ...
  namespace MaterialModel
  {
    ASPECT_REGISTER_MATERIAL_MODEL(AV_3D,
