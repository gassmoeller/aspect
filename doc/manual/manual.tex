...
The plugin and parameter files can be found at \url{benchmarks/melt_column_differenciation/melt_SramekRicardBercovici2007.cc} and \url{benchmarks/melt_column_differenciation/diff_adim.prm}. 
A jupyter notebook (Python 3) for recreating the different figures shown here is provided in \url{benchmarks/melt_column_differenciation/python}. 
...
The plugin \url{benchmarks/melt_column_differenciation/melt_srb.cc} is used as an additional library and needs to be compiled before running the parameter file. 
...
The parameter file is available at \url{benchmarks/melt_column_differenciation/diff_adim.prm}. 
...
The model was designed to solve for very low porosity, and the equations are not be physically valid for porosity high enough -- the matrix is then no longer connected -- then this result was expected.  
