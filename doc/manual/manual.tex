...
\subsubsection{Compaction -- differentiation}


\textit{This section was contributed by Marine Lasbleis.}

Here we use \aspect{} to mode the compaction of a simple column with a mixture of liquid and solid using two-phase flow equations. 
...
The plugin and parameter files can be found at \url{benchmarks/differenciation/melt_srb.cc} and \url{benchmarks/differenciation/diff_adim.prm}. 
A jupyter notebook (Python 3) for recreating the different figures shown here is provided in \url{benchmarks/differenciation/python}. 
...
The global melt model is set up with compressibility, which we will need to remove by setting the parameters to 0. 
We are also neglecting the effects of temperature on the parameters and setting the temperature field to 0 everywhere. 
The viscosities tends to 0 or infinity around $\phi=0$ and $\phi=1$, which is not possible to implement in numerical model. 
We set a maximum viscosity variation due to porosity of $10^6$.

We defined a new material model which incorporate all of these modifications, based on the material model global melt. 
The plugin \url{benchmarks/differenciation/melt_srb.cc} is used as an additional library and needs to be compiled befre running the parameter file. 

\paragraph{Set-up}

We set a column of width one cell, with 200 cells in the y-direction. 
The total size of the system is 1 and all parameters are put to 0 or 1 to have a dimensionless setting. 
...
The parameter file is available at \url{benchmarks/differenciation/diff_adim.prm}. 
...
The agreement is good, with a slight discrepancy at the top, likely due to the top boundary condition not exactly defines at $y=1$.   

\begin{figure}[h]
    \centering
    \includegraphics[width=0.49\textwidth]{delta_1_analytical.pdf}
    \hfill
    \includegraphics[width=0.49\textwidth]{delta_02_analytical.pdf}
...
The porosity profiles evolves such that the liquid is expelled to the top and the matrix compacts at the bottom. 
...
\begin{figure}
    \centering
    \includegraphics[width=0.5\textwidth]{comparison_pts.pdf}
...
Results are similar to \cite{SRPHD07}, except for porosity close to 1, where \aspect{} do not converge correctly and overestimate the segregation. 
Since the model was designed to solve for very low porosity, and that the equations should not be physically valid for porosity high enough than the matrix is no longer connected (the highest threshold usually considered is about 40\%, corresponding to a system made of spheres with a random close packing)
...
\begin{figure}
    \centering
    \includegraphics[width=\textwidth]{delta_1_02.pdf}
