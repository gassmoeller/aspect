...
def process_file( viz_file ):
    ...
    if viz_file == viz_files[-1]:
        ...
        a_vx=[]
        a_vy=[]
    return [viznum,a_T,a_vx,a_vy,T1111,Tslab,Twedge]

results = Parallel(n_jobs=num_cores)(delayed(process_file)(i) for i in viz_files)

Twedge = np.array([x[6] for x in results])
Tslab = np.array([x[5] for x in results])
T1111 = np.array([x[4] for x in results])
