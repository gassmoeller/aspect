          ...
          for (unsigned int q = 0; q < quadrature.size(); q++)
                ...
                !current_constraints.is_constrained(local_dof_indices[q]))
            {
                  ...
                  c_idx <= simulator_access.introspection().component_indices.velocities[dim - 1])
              {
                ...
                if (dim == 2)
                {
                  indicator = prescribed_velocity_indicator_function_2d.value(reinterpret_cast<const Point<2> &>(p), component_direction);
                  u_i = prescribed_velocity_function_2d.value(reinterpret_cast<const Point<2> &>(p), component_direction);
                }
                else
                {
                  indicator = prescribed_velocity_indicator_function_3d.value(reinterpret_cast<const Point<3> &>(p), component_direction);
          ...
          for (unsigned int q = 0; q < quadrature.size(); q++)
                ...
                !current_constraints.is_constrained(local_dof_indices[q]))
            {
              ...
              if (c_idx == simulator_access.introspection().component_indices.pressure)
              {
                ...
                if (dim == 2)
                {
                  indicator = use_vx_indicator ? prescribed_velocity_indicator_function_2d.value(reinterpret_cast<const Point<2> &>(p), 0)
                                               : prescribed_pressure_indicator_function_2d.value(reinterpret_cast<const Point<2> &>(p), 0);
                  pressure = prescribed_pressure_function_2d.value(reinterpret_cast<const Point<2> &>(p), 0);
                }
                else
                {
                  indicator = use_vx_indicator ? prescribed_velocity_indicator_function_3d.value(reinterpret_cast<const Point<3> &>(p), 0)
                                               : prescribed_pressure_indicator_function_3d.value(reinterpret_cast<const Point<3> &>(p), 0);
                  pressure = prescribed_pressure_function_3d.value(reinterpret_cast<const Point<3> &>(p), 0);
                }
