...
for temperature in sub-adiabatic adiabatic super-adiabatic; do
	...
	echo -n "Resolution ala hydrostatic projected-density" > $outputfile
	  for resolution in 0 1 2; do
		  echo '' >> $outputfile
		  echo -n $resolution >> $outputfile
  for formulation in ala hydrostatic projected-density; do #projected-density
    data_folder=output-compressibility-resolution-${resolution}-${formulation}-${temperature}
    cat $data_folder/statistics | tail -n 1 | gawk '{printf " %g",sqrt(($18+$19)*($18+$19))}' >> $outputfile
    done
  done
done
